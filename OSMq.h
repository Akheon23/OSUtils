...
Version status: Tested quickly, buggy?
...
*/
inline int CreateDefaultMq(MQ* pMq)
{
#ifdef _WIN32
	...
	hThread = CreateThread( 
		...
		&dwThreadId); // Return the thread identifier. 
...
Return : EXIT_SUCCESS or EXIT_FAILURE if there is an error.
*/
inline int PostMq(MQ* pMq, void* msg)
{
#ifdef _WIN32
	if (!PostThreadMessage( 
...
there is an error.
*/
inline int WaitAndGetMq(MQ* pMq, void** pMsg, int timeout)//PendMq
{
#ifdef _WIN32
	MSG WIN32msg;
	*pMsg = NULL;
	...
	// that calls WaitAndGetMq()) as data. This will notify the thread to
